...
console = Console()
app = FastAPI(title="AgroCLI Remote Brain")
...
async def broadcast_sensor(data: dict):
    ...
    asyncio.create_task(manager.send_sensor_update(
        data["plant_name"],
        data["moisture"],
        data["temperature"],
        data["humidity"]
...
async def broadcast_pump(data: dict):
    ...
    asyncio.create_task(manager.send_pump_event(
        data["plant_name"],
        data["status"],
        data.get("duration", 0)
    ))
...
async def broadcast_alert(data: dict):
    ...
    asyncio.create_task(manager.send_system_alert(
        data["message"],
        data.get("level", "info")
    ))
    return {"status": "broadcasted"}
